...
class Api_AnimalSpeciesController {
    ...
    private function get(): void {
        if (isset($_GET['id'])){
            $st = $this->db->prepare('SELECT * FROM animal_types WHERE id = ? AND deleted_at IS NULL');
            $st->execute([$_GET['id']]); echo json_encode($st->fetch(PDO::FETCH_ASSOC)); return;
        }
        $q = $this->db->query('SELECT * FROM animal_types WHERE deleted_at IS NULL');
        echo json_encode($q->fetchAll(PDO::FETCH_ASSOC));
    }
    private function post(): void {
        $d = json_decode(file_get_contents('php://input'), true) ?: [];
        $st = $this->db->prepare('INSERT INTO animal_types (name, created_at, updated_at) VALUES (:name, NOW(), NOW())');
    ...
    private function put(): void {
        ...
        $d = json_decode(file_get_contents('php://input'), true) ?: [];
        $st = $this->db->prepare('UPDATE animal_types SET name=:name, updated_at=NOW() WHERE id=:id');
    ...
    private function delete(): void {
        ...
        if (isset($_GET['force']) && $_GET['force']==='true'){
            $st = $this->db->prepare('DELETE FROM animal_types WHERE id=:id'); $st->execute([':id'=>$_GET['id']]);
            return $this->json(['message'=>'Xóa CỨNG species thành công']);
        }
        $st = $this->db->prepare('UPDATE animal_types SET deleted_at = NOW() WHERE id=:id'); $st->execute([':id'=>$_GET['id']]);
